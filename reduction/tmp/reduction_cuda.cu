...
// Single-kernel global memory reduction
__global__ void reduce3(int *d_arr, int n) {
    
    int idx = blockIdx.x * blockDim.x + threadIdx.x;

    // Stride-based reduction (in-place)
    for (int stride = n / 2; stride > 0; stride >>= 1) {
        if (idx < stride) {
            d_arr[idx] += d_arr[idx + stride];
        }
...
int main(int argc, char **argv) {
   ...
   std::cout << "Kernel reduce0 - Final sum: " << h_odata0[0] << ", Time: " << milliseconds/1000 << " s" << std::endl;
   ...
   reduce3<<<blocksPerGrid, threadsPerBlock, threadsPerBlock * sizeof(int)>>>(d_idata,N);
   ...
   std::cout << "Kernel reduce1 - Final sum: " << h_odata0[0] << ", Time: " << milliseconds/1000 << " s" << std::endl;
