...
__global__ void reduce5(int *g_idata, int *g_odata, int n) {
    ...
    // Load data into shared memory with grid stride
    float val = 0.0f;
    while (tid < n) {
        val += g_idata[tid];
    ...
    __syncthreads(); // Synchronize to ensure all warps have written their results

    // Final reduction across the warps
    if (threadIdx.x == 0) {
        float final_sum = 0.0f;
        for (int i = 0; i < blockDim.x / warpSize; ++i) {
            final_sum += sdata[i];
        }
        g_odata[blockIdx.x] = final_sum; // Write the block result to global memory
    }
}
